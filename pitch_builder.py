...
def build_whatsapp_pitch(biz: dict) -> str:
    ...
    if "no_website" in issues:
        return (
            ...
            f"כלומר, לקוחות שמחפשים אתכם עוברים למתחרה.\n\n"
            f"אני בונה אתרים לעסקים קטנים ב-{PRICE_ILS} שקל, כולל עיצוב מלא, "
            f"מותאם לנייד, ו-HTTPS. אשמח לשלוח לכם דמו מותאם לעסק שלכם.\n\n"
    ...
    if len(rest) == 1:
        ...
        body += f"\nמצאתי עוד {len(rest)} בעיות נוספות שפוגעות בביצועי האתר.\n"

    body += (
        f"\nאני בונה אתרים מלאים ב-{PRICE_ILS} שקל שמטפלים בכל הנקודות האלה. "
        f"אשמח לשלוח לכם דמו מותאם לעסק שלכם, ללא התחייבות.\n\n"
        f"{YOUR_NAME} | {YOUR_PHONE}"
    )
    return body
...
def build_full_pitch(biz: dict) -> str:
    ...
    lines += [
        "-" * 48,
        "",
        "אשמח לשלוח לכם דמו מותאם לעסק שלכם — תראו את התוצאה לפני שמחליטים.",
        "",
        f"{YOUR_NAME}",
        f"{YOUR_PHONE}",
        f"{YOUR_WHATSAPP_URL}",
