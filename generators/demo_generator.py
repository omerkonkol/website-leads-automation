...
# ════════════════════════════════════════════════════════════════
#  קריאה ל-Claude — Claude Code CLI (חינם) או SDK
# ════════════════════════════════════════════════════════════════
def _call_claude(prompt: str, api_key: str = "") -> str:
    """
    מנסה לקרוא ל-Claude לפי הסדר:
    1. Claude Code CLI (npx @anthropic-ai/claude-code --print) — חינם עם המנוי הקיים
    2. Anthropic SDK — אם יש API key
    """
    import subprocess, shutil

    # ── שיטה 1: Claude Code CLI ──
    # מחפש npx בנתיבים שכיחים
    npx_paths = [
        shutil.which("npx"),
        r"C:\Program Files\nodejs\npx.cmd",
    ...
    npx = next((p for p in npx_paths if p and os.path.exists(p)), None)

    if npx:
        try:
            result = subprocess.run(
                [npx, "--yes", "@anthropic-ai/claude-code", "--print", prompt],
            ...
            print(f"  [Claude CLI] נכשל ({e}), עובר ל-SDK...")

    # ── שיטה 2: Anthropic SDK ──
    ...
    raise RuntimeError(
        "לא נמצאה דרך לקרוא ל-Claude.\n"
        "פתרון: הפעל את הדשבורד מתוך Claude Code, או הגדר ANTHROPIC_API_KEY."
    )
...
def generate_with_template(
    ...
    # קדימות: Claude Code CLI (חינם) → env var → config.py
...
**החזר HTML מלא בלבד.** התחל ב-<!DOCTYPE html>."""
    ...
    print(f"  Claude מייצר אתר עבור {business_name}...")
    html = _call_claude(prompt, api_key)
